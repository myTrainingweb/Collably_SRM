...
LEFT JOIN SRM.BuyerPO AS row2 ON row2.PONum = row1.POHeader_PONum
AND row2.[LineNo] = row1.PODetail_POLine
AND row2.RelNo = row1.PORel_PORelNum
WHERE row2.PONum IS NULL
OR row2.[LineNo] IS NULL
OR row2.RelNo IS NULL;";
            ...
            //Late
            query = @"select Count(*) from [dbo].[tblPurchaseOrder]
Where RcvDtl_ArrivedDate > PORel_DueDate";
