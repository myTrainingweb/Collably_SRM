...
namespace PlusCP.Models
{
    public class NewPO
    {
        ...
        public bool GetList(DataTable dt, string POStatus)
        {
            ...
            dt.Columns.Add("Updates");
            dt.Columns.Add("Message");
            dt.Columns.Add("BuyerEmailSent");
            dt.Columns.Add("VendorEmailSent");
            dt.Columns.Add("Awaiting");
            dt.Columns.Add("Resent");
            dt.Columns.Add("ResentCreatedDate");
            dt.Columns.Add("ResentVendorEmail");
            DataColumn concatenatedColumn = new DataColumn("PONumber", typeof(string));
            dt.Columns.Add(concatenatedColumn);

            //  Concatenate values from PO, LINE, and REL columns and store in PO-LINE-REL column
            foreach (DataRow row in dt.Rows)
            {
                row["PONumber"] = $"{row["POHeader_PONum"]}-{row["PODetail_POLine"]}-{row["PORel_PORelNum"]}";
            }

            //----------------------BuyerEmailSent Logic
            string Buyersql = "";
            DataTable DtbuyerEmail = new DataTable();
            Buyersql = @"select CONCAT(PONum,+'-'+ [lineNo] + '-' +RelNo) As PONo
                        from[SRM].[BuyerPO] ";
            DtbuyerEmail = oDAL.GetData(Buyersql);
            foreach (DataRow row in dt.Rows)
            {
                string poNo = row["PONumber"].ToString();
                if (DtbuyerEmail.AsEnumerable().Any(dr => dr["PONo"].ToString() == poNo))
                {
                    row["BuyerEmailSent"] = poNo;
                }
            }
            //----------------------------------------

            //----------------------VendorEmailSent Logic
            string Vendorsql = "";
            DataTable DtVendorEmail = new DataTable();
            Vendorsql = @"Select PONo from [SRM].[VendorCommunication] ";
            DtVendorEmail = oDAL.GetData(Vendorsql);
            foreach (DataRow row in dt.Rows)
            {
                string poNo = row["PONumber"].ToString();
                if (DtVendorEmail.AsEnumerable().Any(dr => dr["PONo"].ToString() == poNo))
                {
                    row["VendorEmailSent"] = poNo;
                }
            }
            //----------------------------------------
...
Where GUID NOT IN(Select GUID from [SRM].[VendorCommunication])";
            ...
            // Checking and updating second DataTable
...
WHERE PONo = '<PONo>'
 ";
        ...
        public void UpdateHasActionAHR(string Action, string GUID)
        {
            ...
            oDAL.Execute(sql);
            sql = @"SELECT B.PONum, B.[LineNo], B.relNo, B.PartNo, V.Qty, V.Price, V.DueDate, B.VendorEmail, B.CreatedBy from [SRM].[BuyerPO] B
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                sendVendorEmailAHR(vCreatedBy, vPONo, vPart, vQty, vDueDate, vPrice, vmessage, GUID, vVendorEmail);
            }
        ...
        public void sendVendorEmail(string EuserName, string EPONo, string EpartNo, string EQty, string EDueDate, string EPrice, string Emessage, string GUID)
        {
           ...
           string sql = @"Select VendorEmail From [SRM].[BuyerPO]
            ...
            string recipientEmail = PVendorEmail;
        ...
        public void sendVendorEmailAHR(string EuserName, string EPONo, string EpartNo, string EQty, string EDueDate, string EPrice, string Emessage, string GUID, string VendorEmial)
        {
            // Sender's email address and password
            string senderEmail = "yousufdev4@gmail.com";
            string senderPassword = "qvav wxwd wcho wofj";
            ...
            var subject = "PO Information against this PONo. " + EPONo + "";
            // Recipient's email address
            string recipientEmail = VendorEmial;
