            ...
            foreach (DataColumn col in dt.Columns)
            {
                ...
                if (columnValue is string)
                {
                    ...
                    if (decimal.TryParse((string)columnValue, out decimal decimalValue))
                    {
                        ...
                        if (col.ColumnName.Contains("POHeader_PONum") || col.ColumnName.Contains("PORel_PORelNum") || col.ColumnName.Contains("PODetail_POLine"))
                        {
                ...
                else if (columnValue.GetType() == typeof(DateTime))
                {
                    if (columnValue.ToString().Length == 10 || columnValue.ToString().Contains("12:00:00 AM"))
                        ...
                        columnValue = Convert.ToDateTime(columnValue).ToString("MM/dd/yyyy HH:mm");
                }
